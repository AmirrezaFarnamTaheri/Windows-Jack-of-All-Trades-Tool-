...
try {
    ...
    if (Test-Path $searchDataPath) {
        ...
        Remove-Item -LiteralPath (Join-Path $searchDataPath '*') -Recurse -Force -ErrorAction SilentlyContinue
    }

    Write-Log "Resetting Registry SetupCompletedSuccessfully flag..."
    if (-not (Test-Path $searchRegKey)) { New-Item -Path $searchRegKey -Force | Out-Null }
